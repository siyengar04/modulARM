...
void updatePID()
{
  // Calculate current position and speed
  encoderAngle = (encoderPosition * 360.0) / PULSES_PER_REV;
  calculateSpeed();
  ...
  // Apply motor control (assuming md.setSpeed accepts -255 to 255)
  // Negative values for reverse direction
  md.setSpeed(constrain(Output, -255, 255));
}
